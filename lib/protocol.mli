...
module Command : sig
  ...
  type t =
    ...
    | Disc
        (** Disconnect: server must flush all outstanding commands and then
                will close the connection *)
    | Flush
        (** A flush request or write barrier. All requests received before
                this one will have completed before this command is acknowledged. *)
    | Trim
        (** A hint that a data region is nolonger required and may be
                discarded. *)
    | Unknown of int32
        (** A command which this protocol implementation doesn't
                           suport. *)
  [@@deriving sexp]
...
module Option : sig
  ...
  type t =
    | ExportName
        (** The client would like to connect to a given disk/export by
                     name *)
    | Abort  (** The client would like to quit. *)
    | List
        (** The client would like to receive a list of known
                     disk/exports. *)
...
module Negotiate : sig
  ...
  type t = V1 of v1 | V2 of v2  (** The initial greeting sent by the server *)
...
module Server : sig
  ...
  (** A reponse to a [List] option. Note this option is repeated, once
      per available disk. *)
  type t = {name: string  (** The name of an available disk. *)}
  [@@deriving sexp]
