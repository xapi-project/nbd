let with_command ~command ~strict test =
  ...
  | _ ->
      if strict then
        failwith ("Command " ^ command ^ " not present")
      else
        Printf.printf "!!! Skipping test because command %s is not available"
          command
...
let () =
  Alcotest.run_with_args "Nbd CLI interoperability tests" opts
    [
