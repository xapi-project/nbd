...
module Device = struct
  ...
  let pp_error ppf = function
    ...
    | `Protocol_error e ->
        Fmt.string ppf (Nbd.Protocol.Error.to_string e)
  ...
  let connect uri =
    ...
    | Some "file" ->
        ...
        Block.connect path >|= fun x -> `Local x
    | Some "nbd" -> (
...
module Impl = struct
    ...
    let t =
      ...
      | Result.Error `Unsupported ->
          Printf.fprintf stderr
            "The server does not support the query function.\n%!" ;
          exit 1
      | Result.Error `Policy ->
          Printf.fprintf stderr
            "The server configuration does not permit listing exports.\n%!" ;
          exit 2
    in
    `Ok (Lwt_main.run t)
  ...
  let serve _common filename port exportname certfile curve ciphersuites no_tls
      =
    let tls_role =
      init_tls_get_server_ctx ~curve ~certfile ~ciphersuites no_tls
    in
  ...
  let mirror _common filename port secondary certfile curve ciphersuites no_tls
    ...
    let t =
      ...
      ( Device.connect (Uri.of_string filename) >>= fun primary ->
        (* Connect to the secondary *)
        Device.connect (Uri.of_string secondary) >>= fun secondary ->
...
let size_cmd =
  ...
  let host =
    ...
    Arg.(required & pos 0 (some string) None & info [] ~doc ~docv:"hostname")
  in
  let port =
...
let ciphersuites =
  let doc =
    "Set of ciphersuites for TLS (specified in the format accepted by OpenSSL, \
     stunnel etc.)"
  in
...
let serve_cmd =
  ...
  let exportname =
    let doc =
      {|Export name to use when serving the file. If specified, clients
  ...
  let no_tls =
    let doc =
      "Use NOTLS mode (refusing TLS) instead of the default FORCEDTLS."
    in
...
let mirror_cmd =
  ...
  let man =
    ...
    ; `P "Create a server which allows a client to access a disk using NBD."
    ; `P
        "The server will pass I/O through to a primary disk underneath, while \
         also mirroring the contents to a secondary."
    ; `S "EXAMPLES"
    ]
    @ help
  in
  ...
  let no_tls =
    let doc =
      "Serve using NOTLS mode (refusing TLS) instead of the default FORCEDTLS."
    in
    Arg.(value & flag & info ["no-tls"] ~doc)
  in
  ( Term.(
...
let list_cmd =
  ...
  let man =
    [
      `S "DESCRIPTION"
    ; `P
        "Queries a server and returns a list of known exports. Note older \
         servers may not support the protocol option: this will result in an \
         empty list."
    ]
    @ help
  in
