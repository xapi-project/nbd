...
let with_channels f =
  ...
  let make_channel name (ic, oc) =
    let read c =
      ...
      Lwt_log.debug_f ~section "%s read: %d: %s finished" name len
    ...
    let write c =
      ...
      Lwt_log.debug_f ~section "%s write: %d: %s" name len
        (String.escaped (Cstruct.to_string c))
      >>= fun () ->
      Lwt_io.write_from_string_exactly oc (Cstruct.to_string c) 0 len
      >>= fun () ->
      Lwt_log.debug_f ~section "%s write: %d: %s finished" name len
        (String.escaped (Cstruct.to_string c))
    in
    (write, read)
  in
  ...
  let client_write, server_read =
    make_channel "client -> server" client_to_server
  in
  let server_write, client_read =
    make_channel "server -> client" server_to_client
  in
  ...
  let server_channel =
    Nbd.Channel.
      {
        read_clear= server_read
      ; write_clear= server_write
      ; close_clear= noop
