...
let with_channels f =
  ...
  let make_channel name (ic, oc) =
    let read c =
      Lwt_log.debug_f ~section "%s read: %d" name (Cstruct.len c) >>= fun () ->
      let len = Cstruct.len c in
      let b = Bytes.create len in
      Lwt_io.read_into_exactly ic b 0 len >>= fun () ->
      Cstruct.blit_from_bytes b 0 c 0 len;
      Lwt_log.debug_f ~section "%s read: %d: %s finished" name (Cstruct.len c) (String.escaped (Cstruct.to_string c))
    in
    let write c =
      Lwt_log.debug_f ~section "%s write: %d: %s" name (Cstruct.len c) (String.escaped (Cstruct.to_string c)) >>= fun () ->
      Lwt_io.write_from_string_exactly oc (Cstruct.to_string c) 0 (Cstruct.len c) >>= fun () ->
      Lwt_log.debug_f ~section "%s write: %d: %s finished" name (Cstruct.len c) (String.escaped (Cstruct.to_string c))
    in
    (write, read)
  in
