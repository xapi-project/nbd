...
module Block : Mirage_block.S with type t = Cstruct.t = struct
  ...
  let get_info contents =
    Lwt.return
      Mirage_block.
        {
          read_write= true
        ; sector_size= 1
        ; size_sectors= Cstruct.len contents |> Int64.of_int
        }
  ...
  let read contents sector_start buffers =
    ...
    List.fold_left
      (fun contents buffer ->
        ...
        Cstruct.shift contents (Cstruct.len buffer))
